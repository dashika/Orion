...
router.get('/:id/comment/', function(req, res, next) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query('call get_comments_perscard(?);', [req.param('id')],
            function (err, rows) {
                if (err) {
                    ...
                    {
                        req.session.obj = JSON.parse(rows[0][0].comments);
                    }
                    res.status = 200;
                    connection.commit();
                    connection.release();
...
router.post('/:id/comment/', function(req, res, next) {
    ...
    pool.getConnection(function (err, connection) {
        ...
        connection.query('call insert_comment_perscard(?,?,?);', [req.param('id'), JSON.stringify(req.session.obj), req.body.raiting],
            function (err, rows) {
                if (err) {
                    connection.rollback(function () {
                        connection.release();
